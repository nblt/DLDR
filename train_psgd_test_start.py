...
def main():
    ...
    best = []
    for dldr_start in range(0, 21, 2):
        best_prec1 = 0
        model.load_state_dict(torch.load(os.path.join(args.pretrain_dir,  str(dldr_start) +  '.pt')))
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=0.9)
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                            milestones=[30, 50], last_epoch=args.start_epoch - 1)

        logging.info(f'{dldr_start} Start training: {args.start_epoch} -> {args.epochs}')
        end = time.time()
