...
do
    ...
    CUDA_VISIBLE_DEVICES=0 python -u train_pbfgs.py --epochs=20 --datasets=$datasets --corrupt=$c --params_start=0 --params_end=81  --batch_size=1024 --n_components=40 --arch=$model  --save-dir=save_labelnoise$c\_$model |& tee -a log_$model 

    CUDA_VISIBLE_DEVICES=0 python -u train_psgd.py --epochs=40 --datasets=$datasets --lr=1 --corrupt=$c --params_start=0 --params_end=81  --batch_size=128  --n_components=40 --arch=$model  --save-dir=save_labelnoise$c\_$model |& tee -a log_$model 
done
