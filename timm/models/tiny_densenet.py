...
class TinyBottleneck(nn.Module):
    def __init__(self, in_channels, growth_rate):
        ...
        self.bottle_neck = nn.Sequential(
            nn.BatchNorm2d(in_channels),
            nn.ReLU(inplace=True),
            deConv2d(1, in_channels, inner_channel, kernel_size=1, bias=False),
            nn.BatchNorm2d(inner_channel),
            nn.ReLU(inplace=True),
            deConv2d(1, inner_channel, growth_rate, kernel_size=3, padding=1, bias=False)
        )
...
class TinyTransition(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.down_sample = nn.Sequential(
            nn.BatchNorm2d(in_channels),
            deConv2d(1, in_channels, out_channels, 1, bias=False),
            nn.AvgPool2d(2, stride=2)
        )
...
class TinyDenseNet(nn.Module):
    def __init__(self, block, nblocks, growth_rate=12, reduction=0.5, num_class=100):
        ...
        self.conv1 = deConv2d(1, 3, inner_channels, kernel_size=3, padding=1, bias=False)
        ...
        for index in range(len(nblocks) - 1):
            ...
            self.features.add_module("transition_layer_{}".format(index), TinyTransition(inner_channels, out_channels))
...
@register_model
def tiny_densenet_bc_100_12(pretrained=False, **kwargs):
    model = TinyDenseNet(TinyBottleneck, [16,16,16], growth_rate=12, num_class=10)
