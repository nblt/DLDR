...
def update_param(model, param_vec):
    # Update the model parameters according to param_vec
    ...
    for name,param in model.named_parameters():
        ...
        for i in range(len(list(arr_shape))):
            size *= arr_shape[i]
        param.data = torch.from_numpy(param_vec[idx:idx+size].reshape(arr_shape)).cuda().float()
        idx += size
        assert param.data.shape == arr_shape
